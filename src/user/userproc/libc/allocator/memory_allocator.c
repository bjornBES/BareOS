...
void alloc_new_page()
{
    mmap(heap_end, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    memset((void*)heap_end, 0, PAGE_SIZE);
    heap_size++;
    heap_end = heap_begin + (heap_size * PAGE_SIZE);
}

void allocator_init()
{
    heap_begin = (virt_addr)mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
void *kmalloc(size_t size)
{
	...
	while (mem < last_alloc)
	{
		...
		if (a->size >= size && !(a->status & STATUS_ALLOCATED))
		{
			...
			memory_used += size + ALLOC_ALIGN + sizeof(alloc_t);
