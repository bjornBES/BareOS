...
int pledge_pledge(pledge_flags_t requested)
{
    ...
    {
        requested &= current_process->pledge_mask; // can only restrict further
    }
    log_debug("PLEDGE", "process %u requesting %08b", current_process->pid, requested);
    current_process->pledge_mask = requested;
...
int pledge_check(pledge_flags_t function_pledge)
{
    ...
    if ((current_process->pledge_mask & function_pledge) == function_pledge)
    {
        return RETURN_GOOD;
    }
