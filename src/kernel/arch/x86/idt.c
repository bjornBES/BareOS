...
#ifdef __i686__
void ASMCALL32 IDT_Load_32(IDT_descriptor *descriptor);
#else
void IDT_Load_64(IDT_descriptor *descriptor);
#endif
...
void x86_IDT_set_gate(int interrupt, void *base_ptr, uint32_t segment_descriptor, uint8_t flags, uint8_t ist)
{
    ...
    IDT_table[interrupt].base_3 = (base >> 32);
#endif
}
...
void x86_IDT_load()
{
    IDT_descript.size = sizeof(IDT_table) - 1;
    IDT_descript.ptr = IDT_table;
    
    log_debug(MODULE, "loading IDT");
#ifdef __x86_64__
    IDT_Load_64(&IDT_descript);
#else
    IDT_Load_32(&IDT_descript);
#endif
