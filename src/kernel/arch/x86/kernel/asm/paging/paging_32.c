...
#define PAGE_GLOBAL (1ULL << 7)

#define MODULE "PAGING"

bool paging_print_out = true;

page_table_t kernel_page = 0;
...
void *paging_get_physical(page_table_t page_table_t, virt_addr virtualaddr)
{
    page_directory32 *page_dir = (page_directory32 *)page_table_t.page_dir;
...
void *paging_get_virtual(page_table_t page_table_t, phys_addr physAddr)
{
    page_directory32 *page_dir = (page_directory32 *)page_table_t.page_dir;
...
void paging_map_page(page_table_t page_table_t, virt_addr virtAddr, phys_addr physAddr, uint32_t flags)
{
    ...
    if (paging_print_out)
    {
        log_debug(MODULE, "map_page page_dir=%p virt=%p phys=%p flags=%x pdi=%u pti=%u", page_table_t, virtAddr, physAddr, flags, page_directory_index, page_table_index);
    }
...
void paging_unmap_page(page_table_t page_table_t, virt_addr virtAddr)
{
    ...
    page_table32 *pt = (page_table32 *)paging_get_virtual(page_table_t, frame_addr);
