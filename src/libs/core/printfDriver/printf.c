...
{
    if (gadget->file == -1 || gadget->max_chars == 0)
    {
        return;
    }
...
{
    return function_gadget();
}

// output the specified string in reverse, taking care of any zero-padding
static void out_rev_(output_gadget_t *output, const char *buf, size_t len, size_t width, uint32_t flags)
{
...
static void print_integer_finalization(output_gadget_t *output, char *buf, size_t len, bool negative, numeric_base_t base, size_t precision, size_t width, uint32_t flags)
{
...
static void print_integer(output_gadget_t *output, printf_unsigned_value_t value, bool negative, numeric_base_t base, size_t precision, size_t width, uint32_t flags)
{
...
static void print_broken_up_decimal(
    struct double_components number_, output_gadget_t *output, size_t precision,
    size_t width, uint32_t flags, char *buf, size_t len)
{
    if (precision != 0U)
    {
...
static void print_decimal_number(output_gadget_t *output, double number, size_t precision, size_t width, uint32_t flags, char *buf, size_t len)
{
...
static void print_exponential_number(output_gadget_t *output, double number, size_t precision, size_t width, uint32_t flags, char *buf, size_t len)
{
    ...
    else
    {
        ...
        abs_exp10_covered_by_powers_table = abs(floored_exp10) < PRINTF_MAX_PRECOMPUTED_POWER_OF_10;
    ...
    size_t exp10_part_width = fall_back_to_decimal_only_mode ? 0U : (abs(floored_exp10) < 100) ? 4U
                                                                                               : 5U;
...
static void print_floating_point(output_gadget_t *output, double value, size_t precision, size_t width, uint32_t flags, bool prefer_exponential)
{
...
static uint32_t parse_flags(const char **format)
{
    uint32_t flags = 0U;
    do
    {
...
#define ADVANCE_IN_FORMAT_STRING(cptr_) (cptr_)++
    ...
    while (*format)
    {
        ...
        uint32_t flags = parse_flags(&format);
