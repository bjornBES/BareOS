...
typedef struct {
    ...
    uint8_t numSerialPorts          : 3; // AX bits 11-9 (number of serial ports installed)
    uint8_t numParallelPorts        : 2; // AX bits 15-14
    uint8_t dmaParallelAvailable    : 1; // bit 25 (extended equipment word, may need BX)
    uint8_t reserved                : 6; // fill remaining bits
